...
def basicConfig(level=INFO, filename=None, stream=None, format=None, datefmt=None, style="%"):
    global _level
    _level = level
    if filename:
    ...
    h.setFormatter(Formatter(fmt=format, datefmt=datefmt, style=style))
    root.handlers.clear()
    root.addHandler(h)
...
class Formatter:
    ...
    def __init__(self, fmt=None, datefmt=None, style="%"):
        ...
        self.datefmt = datefmt or "{0:04d}-{1:02d}-{2:02d} {3:02d}:{4:02d}:{5:02d}"
    ...
    def format(self, record):
        if self.style == "%":
            ...
            # The message attribute of the record is computed using msg.format(*args)
